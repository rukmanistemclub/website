<?php
/**
 * Create NYC Testing Timeline Page with Custom Template
...
$existing_page = get_page_by_path('nyc-testing-timeline-when-to-start', OBJECT, 'page');

if ($existing_page) {
    // Update existing page
    $page_data = array(
        'ID'           => $existing_page->ID,
        'post_title'   => 'NYC Testing Timeline: When to Start',
        'post_name'    => 'nyc-testing-timeline-when-to-start',
...
update_post_meta($page_id, '_wp_page_template', 'template-testing-timeline.php');

// Set SEO if Yoast is available
if (function_exists('wpseo_init')) {
    update_post_meta($page_id, '_yoast_wpseo_title', 'NYC Testing Timeline: When to Start Test Prep | NYC STEM Club');
    update_post_meta($page_id, '_yoast_wpseo_metadesc', 'Strategic preparation guidance for SHSAT, ISEE, SAT, and ACT success. Learn when to start test prep for optimal results.');
    update_post_meta($page_id, '_yoast_wpseo_focuskw', 'NYC Testing Timeline');
}
...
echo '<p style="font-size: 18px; line-height: 1.6;">NYC Testing Timeline page <strong>' . $action . '</strong> successfully with custom template!</p>';
echo '<p><strong>Page ID:</strong> ' . $page_id . '</p>';
echo '<p><strong>Template:</strong> template-testing-timeline.php (Custom Full Width)</p>';
...
echo '<p style="margin-top: 30px; padding: 15px; background: #fff7ed; border-radius: 6px; font-size: 14px;">💡 <strong>Note:</strong> The page uses a custom template with interactive ISEE levels section, timeline signposts, and responsive design.</p>';
echo '</div>';

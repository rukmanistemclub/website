<?php
/**
 * Create SAT & ACT Test Prep Page with Custom Template
...
$existing_page = get_page_by_path('sat-act-test-prep', OBJECT, 'page');

if ($existing_page) {
    // Update existing page
    $page_data = array(
        'ID'           => $existing_page->ID,
        'post_title'   => 'SAT & ACT Test Prep',
        'post_name'    => 'sat-act-test-prep',
...
update_post_meta($page_id, '_wp_page_template', 'template-sat-act-prep.php');

// Set SEO if Yoast is available
if (function_exists('wpseo_init')) {
    update_post_meta($page_id, '_yoast_wpseo_title', 'SAT & ACT Test Prep - Achieve Your Dream College Score | NYC STEM Club');
    update_post_meta($page_id, '_yoast_wpseo_metadesc', '96% of students improve their scores with our proven SAT & ACT prep program. Average 6-9 point ACT increase and 100+ point SAT increase. Expert instructors, personalized approach.');
    update_post_meta($page_id, '_yoast_wpseo_focuskw', 'SAT ACT Test Prep');
}
...
echo '<p style="font-size: 18px; line-height: 1.6;">SAT & ACT Test Prep page <strong>' . $action . '</strong> successfully with custom template!</p>';
echo '<p><strong>Page ID:</strong> ' . $page_id . '</p>';
echo '<p><strong>Template:</strong> template-sat-act-prep.php (Custom Full Width)</p>';
...
echo '<p style="margin-top: 30px; padding: 15px; background: #fff7ed; border-radius: 6px; font-size: 14px;">💡 <strong>Note:</strong> The page includes hero section with track record card, trust bar, comprehensive content sections, benefit cards, FAQ accordion, and Trustindex testimonials.</p>';
echo '</div>';
